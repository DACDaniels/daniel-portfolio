 ...
 */
function prefersReducedMotionOnLoad(): boolean {
...
export function useScrollReveal<T extends HTMLElement = HTMLElement>(
  ...
  // Lazy initial state: when reduced motion is preferred, start
  // already revealed. This avoids a synchronous setState inside the
  // effect (which React 19's react-hooks/set-state-in-effect lint
  // flags) and removes a wasted re-render for those users.
  const [revealed, setRevealed] = useState<boolean>(prefersReducedMotionOnLoad);
